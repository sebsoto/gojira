...
func NewRelease(releaseplan, version string, jiraProjects []string) error {
	...
	fmt.Printf("Merges not included in release:\n")
	for _, mergeCommit := range mergesSinceRelease {
		fmt.Printf("%s\n", mergeCommit.GetMessage())
	}
	...
	r, err := newRelease("windows-machine-config-operator-releasev1alpha1-4-18", jiraTickets)
	if err != nil {
		return err
	}
...
func newRelease(component string, jiraIssues []*jira.Issue) (*releasev1alpha1.Release, error) {
	var cveNames []string
	...
	for _, jiraIssue := range jiraIssues {
		if !strings.HasPrefix(jiraIssue.Fields.Summary, "CVE-") {
